...
1. **計器であって観測基盤ではない。** ログ蓄積・ダッシュボードは作らない。「パッと見」に特化する(コスト系は当日合計の推定表示のみ例外として持つ)
...
バージョンを固定したい場合は、npm なら `tachograph@0.4.0`、go なら `@v0.4.0` のように指定します。`tacho version` はインストール時に埋め込んだ版(タグ)を表示します — タグ無しのローカルビルドはコミットベースの擬似バージョンになります。
...
| `5h.pct` / `wk.pct` | 5時間枠 / 週次枠のリミット**残量**(残り割合)、`76%` |
| `5h.bar:8` / `wk.bar:8` | 指定幅の残量ゲージ(使うほど減る)、`██████░░` |
| `5h.dial` / `wk.dial` | 1文字の残量ダイヤル、`○◔◑◕●`(● = 全部残っている。データ無しは `◌`) |
| `5h.moon` / `wk.moon` | 大きめの月齢残量ダイヤル、`🌑🌒🌓🌔🌕`(🌕 = 全部残っている。絵文字のため色分け対象外。データ無しは `◌`) |
